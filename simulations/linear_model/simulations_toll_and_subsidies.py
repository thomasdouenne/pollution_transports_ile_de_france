...
for subsidy in range(0,101):
    ...
    while t_is_pref == 1:
        n_t = n_t + 0.00001
        left = p_v + nu * (p_t + alpha * N * n_t) * (1 + mu * (1 - n_d - n_t))
...
df_sim['r_'] = p_t + df_sim['index'] # Check that the distributon is consistent with the value of alpha
...
for toll in [0,10]:
    ...
    df_sim['b_v_{}'.format(toll)] = (
        df_sim['r_'] - p_v - monthly_toll - nu * (1 + mu * df_toll['n_v_toll'][toll]) * df_sim['r_']
        )
    df_sim['b_t_{}'.format(toll)] = (
        df_sim['r_'] - p_t - theta * (1 + gamma * df_toll['n_t_toll'][toll]) * df_sim['r_']
        )

    df_sim['b_option_{}'.format(toll)] = (
        df_sim['b_d_{}'.format(toll)] * (df_sim['b_d_{}'.format(toll)] > df_sim['b_v_{}'.format(toll)]) * (df_sim['b_d_{}'.format(toll)] > df_sim['b_t_{}'.format(toll)])
        + df_sim['b_v_{}'.format(toll)] * (df_sim['b_v_{}'.format(toll)] > df_sim['b_d_{}'.format(toll)]) * (df_sim['b_v_{}'.format(toll)] > df_sim['b_t_{}'.format(toll)])
        + df_sim['b_t_{}'.format(toll)] * (df_sim['b_t_{}'.format(toll)] > df_sim['b_d_{}'.format(toll)]) * (df_sim['b_t_{}'.format(toll)] > df_sim['b_v_{}'.format(toll)])
        )
    ...
    if toll != 0:
        ...
        df_sim['option'] = (
            1 * (df_sim['b_d_{}'.format(toll)] > df_sim['b_v_{}'.format(toll)]) * (df_sim['b_d_{}'.format(toll)] > df_sim['b_t_{}'.format(toll)])
            + 2 * (df_sim['b_v_{}'.format(toll)] > df_sim['b_d_{}'.format(toll)]) * (df_sim['b_v_{}'.format(toll)] > df_sim['b_t_{}'.format(toll)])
            + 3 * (df_sim['b_t_{}'.format(toll)] > df_sim['b_d_{}'.format(toll)]) * (df_sim['b_t_{}'.format(toll)] > df_sim['b_v_{}'.format(toll)])
            )



df_sim['diff_v_t'] = df_sim['b_t_0'] - df_sim['b_v_0']
...
# Create graphs - equilibrium Q_d - tolls
plt.plot(df_sim['r_'], df_sim['gain_10'])
plt.xlabel('Agents income')
plt.ylabel('gains_losses')
plt.grid(True)
plt.show()
