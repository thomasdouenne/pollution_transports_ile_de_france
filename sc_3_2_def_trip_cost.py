...
def define_insurance_cost_vehicle_random_uniform(data): # Impute cost insurance : average in intervals
    ...
    data['insurance_cost_hh'] = (
        ...
        + (0 + 400)/2 * (data['asscout'] == '1')
        + (400 + 800)/2 * (data['asscout'] == '2')
        + (800 + 1200)/2 * (data['asscout'] == '3')
        + (1200 + 1600)/2 * (data['asscout'] == '4')
        + (1600 + 2000)/2 * (data['asscout'] == '5')
        + (2000 + 5000)/2 * (data['asscout'] == '6') # On fixe un maximum à 5,000
        + (0 + 1000)/2 * (data['asscout'] == '8') # We set between 0 and 1,000 for those who don't know
...
def define_maintenance_cost_vehicle_random_uniform(data): # Impute cost vehicle  : average in intervals
    ...
    data['maintenance_cost_hh'] = (
        ...
        + (0 + 400)/2 * (data['ancout'] == '1')
        + (400 + 800)/2 * (data['ancout'] == '2')
        + (800 + 1200)/2 * (data['ancout'] == '3')
        + (1200 + 1600)/2 * (data['ancout'] == '4')
        + (1600 + 2000)/2 * (data['ancout'] == '5')
        + (2000 + 5000)/2 * (data['ancout'] == '6') # On fixe un maximum à 5,000
        + (0 + 1000)/2 * (data['ancout'] == '8') # We set between 0 and 1,000 for those who don't know
        ) / 12 # We divide to get monthly cost from annual one
...
def define_share_public_transport_card_paid(data):
    data['share_tc_card_paid'] = (
        ...
        + (0 + 0.25)/2 * (data['rembtc'] == '2')
        + 0.25 * (data['rembtc'] == '3')
        + (0.26 + 0.5)/2 * (data['rembtc'] == '4')
        + 0.5 * (data['rembtc'] == '5')
        + (0.51 + 0.75)/2 * (data['rembtc'] == '6')
        + 0.75 * (data['rembtc'] == '7')
        + (0.76 + 1)/2 * (data['rembtc'] == '8')
        + 1 * (data['rembtc'] == '9')
        + (0 + 1)/2 * (data['rembtc'] == '10') # We set between 0 and 1,000 for those who don't know
        )
